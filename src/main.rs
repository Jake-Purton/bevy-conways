use bevy::{prelude::*, render::{render_resource::{Extent3d, TextureDimension, TextureFormat, TextureUsages}, texture::ImageSampler}, asset::HandleId};
use rand::Rng;
...
fn swap_colours (
    ...
    // if keys.just_pressed(KeyCode::Space) {
        ...
        if let Some(image) = images.get_mut(&handle) {
            ...
            for _ in 0..(image.data.len() / 4) {
                new_image.push(0);
            }

            let old_image: Vec<u8> = image.data.iter().enumerate().filter(|(i, _)| (i + 1) % 4 == 0 ).map(|x| *x.1).collect();
